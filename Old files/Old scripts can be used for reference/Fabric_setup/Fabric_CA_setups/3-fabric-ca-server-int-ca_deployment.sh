...
cd ~/fabric-ca-client/tls-ca/ica-admin/msp/keystore/
tempCert="$(ls -t * | head -1)"

# Changing the name of the key file to key.pem
mv ~/fabric-ca-client/tls-ca/ica-admin/msp/keystore/$tempCert ~/fabric-ca-client/tls-ca/ica-admin/msp/keystore/key.pem

# Copying the ica-admin TLS-certificate and private key

cd ~/fabric-ca-server-int-ca/
cp ../fabric-ca-client/tls-ca/ica-admin/msp/signcerts/cert.pem tls
cp ../fabric-ca-client/tls-ca/ica-admin/msp/keystore/key.pem tls
...
./fabric-ca-server init -b ica-admin:ica-Welkom01!
...
sed -i '427s\.*\    url: https://rca-admin:'rca-Welkom01!'@localhost:7055\' fabric-ca-server-config.yaml
sed -i '428s\.*\    caname: Spark!LivingLab-CA\' fabric-ca-server-config.yaml
...
rm -rf ~/fabric-ca-server-int-ca/msp
