...
export class SmartContractUtil {
    ...
    public static async getConnectionProfile() {
        ...
        const connectionProfilePath: string = path.join(homedir, '.fabric-vscode', 'v2', 'environments', 'blockchain', 'gateways', 'Org1 Gateway.json');
    ...
    public static async submitTransaction(contractName: string, functionName: string, args: string[], gateway): Promise<Buffer> {
        ...
        if (contractName !== '') {
            contract = await network.getContract('blockchain-backend', contractName);
        } else {
            contract = await network.getContract('blockchain-backend');
        }
